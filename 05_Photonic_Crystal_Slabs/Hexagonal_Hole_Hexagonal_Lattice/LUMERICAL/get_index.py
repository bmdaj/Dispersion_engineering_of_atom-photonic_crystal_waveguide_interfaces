...
def create_fsp_file(i, name):
    ...
    """
    meshacc = 5  # Bigger is better but slower
    ...
    wMarginXY = 1e-6  # Margin width where there will be no holes in the supercell
    ...
    timefactor = 1  # as in t = (time factor) * lZ / c - actually use 2PhC XY-size
    ...
    if (aPhC - tTether) > 1e-9:
        RHole = (aPhC - tTether) / np.sqrt(
            3
        )  # so this means that R_hole = multiplier * 2 / np.sqrt(3)
        print(RHole)
...
def get_results(i, name):
    ...
    freq = 1.0 * fcs_d2 + 0.0 * fcs_d1
    bandwidth = 150e12  # This is giant. will it work?
    freqhighres = 800  # For more detailed plots
...
for i in range(1):
    ...
    result = get_results(i, name)

    np.save("results/index" + name, np.array(result))
