...
def create_fsp_file(i, name):
    ...
    """
    meshacc = 3  # Bigger is better but slower

    # SCALING FACTOR

    freq_r = 370.87e12
    freq = 351.726e12
    scaling_factor = freq_r / freq

    wSimVolXY = 8.0e-6 * scaling_factor  # The 2D PhC plane
    wSimVolZ = (
        3.0e-6 * scaling_factor
    )  # propagation direction (okay it's also thickness direction)
    ...
    tSiN = 200e-9  # Thickness of SiN layer: SHOULD BE FIXED!!!!

    # OTHER PARAMETERS

    wMarginXY = (
        0.5e-6 * scaling_factor
    )  # Margin width where there will be no holes in the supercell
    nMarginIndex = 1.997  # Refractive index of Si3N4
    aPhC = 383.65e-9 * scaling_factor  # Lattice constant
    tTether = 170e-9 * scaling_factor
    ...
    freqhighres = 2000  # For more detailed plots

    timefactor = 1000.0  # as in t = (time factor) * lZ / c - actually use 2PhC XY-size
