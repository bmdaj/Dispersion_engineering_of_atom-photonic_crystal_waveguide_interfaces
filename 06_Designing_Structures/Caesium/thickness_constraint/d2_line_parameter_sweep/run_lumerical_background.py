...
def create_fsp_file(i, name, aPhC, multiplier):
    ...
    wSimVolXY = 7.0e-6  # 8.0e-6  # The 2D PhC plane
    wSimVolZ = 3.0e-6  # propagation direction (okay it's also thickness direction)

    c = 3e8  # Speedq of  light
    ...
    freqhighres = 400  # Normally, 2000 For more detailed plots

    timefactor = 200.0  # Normally, 1000 as in t = (time factor) * lZ / c - actually use 2PhC XY-size
    ...
    L_rad = 13.5 * aPhC  # Normally 14
    h_rad = 0.5 * tSiN + 2 * aPhC  # 0.5 * tSiN + 26 * aPhC
    w_rad = 1 * aPhC  # 4.0 * aPhC
...
multiplier_list = np.linspace(0.1, 0.4, 10)
# %%
for i in range(1):  # for all three dipole orientations
    for j in range(len(aPhC_list)):
        for k in range(len(multiplier_list)):
            if (j == 4 and k == 8) or (j == 8 and k == 4):
                name = names[i] + "_" + str(j) + "_" + str(k)

                aPhC = aPhC_list[j]
                multiplier = multiplier_list[k]

                print(i, j, k)
                print(aPhC, multiplier)

                create_fsp_file(i=i, name=name, aPhC=aPhC, multiplier=multiplier)
