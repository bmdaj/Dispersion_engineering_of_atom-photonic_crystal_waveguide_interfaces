...
def calculate_ifield(sim_params, num_period, custom_material = False):
    ...
    for f in efields:
        ...
        f1 = (efieldx*np.conjugate(efieldx)+efieldy*np.conjugate(efieldy)+efieldz*np.conjugate(efieldz)) 
...
def plot_epsilon_YZ (sim_params, a, num_period, ylim,  zlim, title, custom_material=False):
...
def plot_epsilon_XY (sim_params, a, num_period, xlim,  ylim, title, custom_material=False):
    ...
    ax.imshow(converted_eps[xmin_index:xmax_index,ymin_index:ymax_index,z_half_index].T, extent = extent, interpolation='spline36', cmap='binary')
    ax.set_ylabel('Y distance')
...
def plot_field_2D_XY (eps, field, a, resolution, num_band, xlim,  ylim, title):
    ...
    ax.contour(eps[xmin_index:xmax_index,ymin_index:ymax_index,z_half_index], extent=extent, cmap='binary')
    
    posd1 = ax.imshow(np.real(field[num_band][xmin_index:xmax_index,ymin_index:ymax_index]), extent=extent, interpolation='spline36', cmap='plasma')
...
def plot_field_2D_XZ (eps, field, a, resolution, num_band, xlim,  zlim, title):
    ...
    ax.contour(eps[xmin_index:xmax_index,y_half_index,zmin_index:zmax_index], extent=extent, cmap='binary')
    
    posd1 = ax.imshow(np.real(field[num_band][xmin_index:xmax_index,zmin_index:zmax_index]), extent=extent, interpolation='spline36', cmap='plasma')
...
def plot_unit_cell_cross_sections(a, sim_params, custom_material = False):
    ...
    extent1 = [-y_half_index * distance_conversion, y_half_index * distance_conversion, -y_half_index * distance_conversion, y_half_index * distance_conversion] 
    
    pos1 = ax1.imshow(eps_data[:,:,z_half_index], extent=extent1, interpolation='spline36', cmap='binary')
    
    extent2 = [-y_half_index * distance_conversion, y_half_index * distance_conversion, -dz_index * distance_conversion, dz_index * distance_conversion] 

    
    pos2 = ax2.imshow(np.rot90(eps_data[:,y_half_index,z_half_index-dz_index:z_half_index+dz_index],2).T, extent=extent2, interpolation='spline36', cmap='binary')
...
def hexagonal_lattice_bands (a, files):
    ...
    for i in range(1,7):
        ...
        if i == 1:
            ax.plot(np.linspace(0,len(f1[:,1])-1, len(f1[:,1])), f1[:,-i]*(w_to_n), c='black', linestyle='dashed', label = 'Dispersion modes') 
        else:
            ax.plot(np.linspace(0,len(f1[:,1])-1, len(f1[:,1])), f1[:,-i]*(w_to_n), c='black', linestyle='dashed') 


    ax.plot(np.linspace(0,len(f2[:,1])-1, len(f2[:,1])), f2[:,-1]*(w_to_n), c='orange', label='TE mode')
    ax.plot(np.linspace(0,len(f2[:,1])-1, len(f2[:,1])), f2[:,-2]*(w_to_n), c='orange') 
    ax.plot(np.linspace(0,len(f3[:,1])-1, len(f3[:,1])), f3[:,-1]*(w_to_n), c='blue', label='TM mode') 
    ax.plot(np.linspace(0,len(f3[:,1])-1, len(f3[:,1])), f3[:,-2]*(w_to_n), c='blue') 
    ...
    ax.fill_between(k_point, np.max(f2[:,-2])*np.ones_like(k_point)*(w_to_n), np.min(f2[K_index:M_index,-1])*np.ones_like(k_point)*(w_to_n), color='orange', alpha=0.5, label='TE Gap') 
