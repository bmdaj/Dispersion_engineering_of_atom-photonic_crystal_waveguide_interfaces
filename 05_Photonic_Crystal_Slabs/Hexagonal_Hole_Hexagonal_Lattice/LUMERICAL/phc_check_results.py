...
ax.plot(fUse, Purcell_X, color="red", lw=2, label="X orientation")
ax.plot(fUse, Purcell_Y, color="blue", lw=2, label="Y orientation")
ax.plot(fUse, Purcell_Z, color="green", lw=2, label="Z orientation")
...
ax.legend(loc="best", frameon=True, fontsize=38)
...
ax.plot(fUse, Purcell_rad_X, color="red", lw=2, label="X orientation")
ax.plot(fUse, Purcell_rad_Y, color="blue", lw=2, label="Y orientation")
ax.plot(fUse, Purcell_rad_Z, color="green", lw=2, label="Z orientation")

# ax.set_yscale("log")
ax.legend(loc="best", frameon=True, fontsize=40)
...
print("Rad. emission min X : ", np.min(Purcell_rad_X))
print("Rad. emission min Y :", np.min(Purcell_rad_X))
...
Green_function_2 = np.load("results/Green_functions_phc_2_point.npy")

T_1_2 = (
    1
    / np.sqrt(2)
    * (Green_function_2[:, :, 0, index, 0] + 1j * Green_function_2[:, :, 0, index, 1])
)
...
im = ax.imshow(
    np.real(T_1_2)[index_low:index_high, index_low:index_high].T,
    vmax=2.5,
    vmin=-2.5,
    cmap="seismic",
    extent=[-zoom_extent, zoom_extent, -zoom_extent, zoom_extent],
