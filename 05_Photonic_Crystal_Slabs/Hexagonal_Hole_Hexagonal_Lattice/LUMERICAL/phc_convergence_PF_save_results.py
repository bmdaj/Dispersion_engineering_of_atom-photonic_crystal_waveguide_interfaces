...
def get_results(params, i):
    ...
    fdtd.load(
        "lumerical-running-savefile" + "_convergence_phc_mesh_acc_" + str(int(mesh_acc))
    )
...
f = open("convergence_phc_PF.log", "w")  # logfile to track progress of simulation
...
for i in range(len(mesh_accs)):
    ...
    np.savetxt(
        "convergence_results/phc_PF_mesh-acc_"
        + str(mesh_acc)
        + "_time-factor_"
        + str(time_factor)
        + "_nfreq_"
