...
index = list(Purcell_X).index(np.max(Purcell_X))
print("Freq. for maximum in X: ", fUse[index])
...
value = find_nearest(fUse, fcs * 1e-12)
...
print("Rad. emission in Y for trans. freq ", Purcell_Y[index])
print("Rad. emission min. in X: ", np.min(Purcell_rad_X))
print("Rad. emission min. in Y:", np.min(Purcell_rad_Y))
index = list(Purcell_rad_X).index(np.min(Purcell_rad_X))
print("Freq. for min in X: ", fUse[index])
...
# print("Max. beta factor Z: ", np.max(beta_factor_Z))
...
# print("Max. Gamma2D/Gamma' in Z: ", np.max(Gamma_1D_vs_prime_Z))
print("Min. Gamma2D/Gamma' in X: ", np.min(Gamma_1D_vs_prime_X))
print("Min. Gamma2D/Gamma' in Y: ", np.min(Gamma_1D_vs_prime_Y))
index = list(Gamma_1D_vs_prime_X).index(np.max(Gamma_1D_vs_prime_X))
print("Freq. for maximum in X: ", fUse[index])
...
value = find_nearest(fUse, 166.551)
index = list(fUse).index(value)
...
wSimVolXY_tot = 30.0e-6
zoom_extent = 30 * 60 / np.shape(index_z)[0]
