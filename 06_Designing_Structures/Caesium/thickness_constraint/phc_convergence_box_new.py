...
def create_fsp_file(params, i, num_iter):
    ...
    wSimVolXY = 7.0e-6  # The 2D PhC plane
...
a = 383.65e-9  # Lattice constant of structure
tSiN = 200e-9
L_rads = np.array([2, 4, 8, 9, 10, 11, 12, 13, 13.5]) * a
h_rads = (
    np.ones(len(L_rads)) * 0.5 * tSiN
    + np.array([0.5, 1, 1.5, 2, 2.5, 3.0, 3.5, 4.0, 4.5]) * a
)
w_rads = np.array([0.1, 0.3, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2.0]) * a
